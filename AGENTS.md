...
### Playbook: TeslaMate's Grafana (`teslamate/grafana:latest`)

Full incident: `ops-log.md` ("2026-07-24 — TeslaMate Grafana: Broken Datasource, Then Locked-Out Admin").

- This image currently resolves to **Grafana v13.0.1+security-01** — much newer than the image has historically shipped, via unpinned `:latest` drift. Classic file-based **datasource provisioning silently doesn't run** on this version (dashboard/alerting provisioning both still work fine, so it's not a total provisioning failure — just datasources specifically). `GF_FEATURE_TOGGLES_provisioning=true` (set in `teslamate.yaml`) does **not** fix this — it only exposes a new `provisioning.grafana.app` API group, unrelated to the classic mechanism. Don't re-investigate this toggle; it's a dead end.
- If a panel/dashboard shows *"You do not currently have a default database configured for this data source"* even though the datasource exists and health-checks OK: the fix is the **Database name** field on the datasource's settings page (Connections → Data sources → TeslaMate) — set it to `teslamate` and Save & test. Root cause: newer Postgres-plugin versions read the database name from `jsonData.database`, not the legacy top-level `database` field, and an older/pre-existing datasource entry can have only the legacy field populated.
- **To run any one-off `grafana cli ...` command on this image, use `docker exec <live running container> grafana cli ...` — never a fresh `docker run`.** This image's entrypoint (`/run.sh`) does not treat `grafana cli ...` passed as the container command as a one-shot CLI invocation; it ignores that and boots a second full Grafana server instead, which then sits there indefinitely (looks exactly like a hang from the caller's side — it isn't, `docker ps` will show it as a live, healthy, fully-booted server). If this happens, `docker kill` the orphaned container manually.
- `GF_AUTH_BASIC_ENABLED=false` on this instance — HTTP Basic Auth always 401s regardless of correct credentials. For scripted API access, log in via `POST /login` with a JSON body (`{"user":...,"password":...}`), save the session cookie, and reuse it for subsequent API calls.
- Avoid scaling `teslamate_grafana` to 0 and back mid-investigation unless truly necessary — doing so while a password change was in flight coincided with a lockout during this incident (unconfirmed causal mechanism, but the correlation was suspicious enough to note here).
